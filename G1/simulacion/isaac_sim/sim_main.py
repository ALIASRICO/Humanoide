...
parser.add_argument("--no_image_server", action="store_true", default=False, help="skip image server (for debugging GUI mode)")
...
parser.add_argument("--skip_stiffness_check", action="store_true", default=False, help="skip robot stiffness/damping check at startup (useful in headless test mode)")
...
# In test/headless-smoke-test mode (--skip_stiffness_check), disable
# synchronous renderer wait so app.update() returns without blocking on
# RTX shader compilation for new GPU architectures (e.g. RTX 5090 Blackwell).
if getattr(args_cli, "skip_stiffness_check", False):
    try:
        import carb
        _cs = carb.settings.get_settings()
        _cs.set("/app/renderer/waitIdle", False)
        _cs.set("/app/hydraEngine/waitIdle", False)
        _cs.set("/app/updateOrder/checkForHydraRenderComplete", 0)
    except Exception:
        pass
...
def main():
    ...
    # Only in headless mode: in GUI mode this blocks the render loop too long
    # and GNOME's _NET_WM_PING watchdog (~5s) kills the process with SIGKILL
    _headless_for_stiffness = bool(getattr(args_cli, "headless", False)) and not bool(getattr(args_cli, "skip_stiffness_check", False))
    if _headless_for_stiffness:
        print("\n" + "="*60)
        ...
        try:
            ...
            if stiffness_data:
                ...
                print("⚠️ Failed to get robot parameters")
        except Exception as e:
    ...
    if not args_cli.replay_data:
        if not getattr(args_cli, 'no_image_server', False):
            ...
            print("========= image server skipped (--no_image_server) =========")
            image_server = None
        print("========= create dds =========")
        try:
