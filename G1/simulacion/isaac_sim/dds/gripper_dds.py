...
class GripperDDS(DDSObject):
    ...
    def setup_publisher(self) -> bool:
        ...
        try:
            ...
            # Brief pause: reusing the same IDL type (MotorStates_) for a second topic immediately
            # after the first DataWriter's Init() can crash inside ddspy_topic_create() in the
            # C layer. A short wait lets the DDS type-registration threads settle.
            time.sleep(0.5)
