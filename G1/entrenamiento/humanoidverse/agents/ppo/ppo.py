...
class PPO(BaseAlgo):
    ...
    def evaluate_policy(self):
        ...
        # Set a forward walking command so we can verify the policy direction
        if hasattr(self.env, 'commands'):
            self.env.commands[:, 0] = 0.5   # 0.5 m/s forward (x)
            self.env.commands[:, 1] = 0.0   # no lateral
            self.env.commands[:, 2] = 0.0   # no yaw
        ...
        while True:
            ...
            if done_mask.any():
                completed_episodes.extend(episode_rewards[done_mask].tolist())
                episode_rewards[done_mask] = 0.0
            if step % LOG_INTERVAL == 0:
                sim = self.env.simulator
                height = sim.base_pos[:, 2].mean().item()
                reward_now = rewards.mean().item()
                n_done = len(completed_episodes)
                avg_ep_rew = sum(completed_episodes[-50:]) / max(len(completed_episodes[-50:]), 1)
                if hasattr(self.env, 'commands'):
                    cmd = self.env.commands[:, :2]
                    actual_vel = sim.base_lin_vel[:, :2]
                    tracking_err = torch.norm(cmd - actual_vel, dim=1).mean().item()
                    logger.info(
                        f"[EVAL step {step:5d}] "
                        f"rew={reward_now:+.3f}  "
                        f"vel_err={tracking_err:.3f}  "
                        f"height={height:.3f}m  "
                        f"episodes={n_done}  "
                        f"avg_ep_rew={avg_ep_rew:.2f}"
                    )
                else:
                    sim = self.env.simulator
                    dof = sim.dof_pos[0]  # primer env
                    lk = dof[3].item() if dof.shape[0] > 3 else 0.0
                    rk = dof[9].item() if dof.shape[0] > 9 else 0.0
                    lh = dof[0].item() if dof.shape[0] > 0 else 0.0
                    # fase desde el env si está disponible
                    phase_str = ""
                    if hasattr(self.env, 'phase_signal'):
                        ph = self.env.phase_signal[0].item()
                        alpha = (ph + 1.0) / 2.0
                        phase_str = f"  α={alpha:.2f}"
                    logger.info(
                        f"[EVAL step {step:5d}] "
                        f"rew={reward_now:+.3f}  "
                        f"height={height:.3f}m{phase_str}  "
                        f"Lknee={lk:+.3f}  Rknee={rk:+.3f}  Lhip={lh:+.3f}  "
                        f"episodes={n_done}  avg_ep_rew={avg_ep_rew:.2f}"
                    )
            step += 1
